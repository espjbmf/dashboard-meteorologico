...
// Função que será chamada toda vez que um novo dado chegar
function atualizarPagina(data) {
    if (!data) return; // Se não houver dados, não faz nada

    // --- Dados Meteorológicos ---
    document.getElementById('temp-externa-valor').innerHTML = data.temperatura + '<span> &deg;C</span>';
    ...
    // --- Dados Internos e Calculados ---
    // Simulação da temp. interna, pois ela não vem do Firebase
    const temperaturaInterna = data.temperatura ? (parseFloat(data.temperatura) + 2.5).toFixed(1) : '--';
    const pontoDeOrvalho = calcularPontoOrvalho(parseFloat(data.temperatura), parseFloat(data.umidade));
    ...
    // --- Sumário do Tempo ---
    const { texto, icone } = analisarCondicoes(data);
...
function analisarCondicoes(data) {
    const temperatura = parseFloat(data.temperatura);
    const umidade = parseFloat(data.umidade);
    const vento = parseFloat(data.velocidade_vento);
    const pontoOrvalho = calcularPontoOrvalho(temperatura, umidade);
